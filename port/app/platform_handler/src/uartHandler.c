...
static xSemaphoreHandle uart_dma_sem;
...
void DATA_UART_Configuration(void) {
    ...
    // Initialize DATA1 UART DMA channel
    dma_uart_tx[SEG_DATA1_CH] = dma_claim_unused_channel(true);
    dma_uart_c[SEG_DATA1_CH] = dma_channel_get_default_config(dma_uart_tx[SEG_DATA1_CH]);
    channel_config_set_transfer_data_size(&dma_uart_c[SEG_DATA1_CH], DMA_SIZE_8);
    channel_config_set_dreq(&dma_uart_c[SEG_DATA1_CH], uart_get_dreq(uart_id[SEG_DATA1_CH], true));

    uart_dma_sem = xSemaphoreCreateMutex();
...
int32_t platform_uart_puts_dma(uint8_t* buf, uint16_t bytes, int channel) {
    ...
    xSemaphoreTake(uart_dma_sem, portMAX_DELAY);

    dma_channel_configure(dma_uart_tx[SEG_DATA0_CH], &dma_uart_c[SEG_DATA0_CH],
                          &uart_get_hw(DATA0_UART_ID)->dr,
                          buf,
                          bytes,
                          true);
