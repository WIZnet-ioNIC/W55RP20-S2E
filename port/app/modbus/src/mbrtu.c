...
void eMBRTUInit(uint32_t ulBaudRate) {
    ...
    */
    if (baud_table[ulBaudRate] > 19200) {
        t35_time_us = 1750; // Fixed value: 1750μs
        usTimerT35_50us = 35; // 1750us / 50us = 35
    } else {
        /* Calculate 1 bit time (μs) = 1,000,000 / baudrate */
        uint32_t bit_time_us = 1000000UL / baud_table[ulBaudRate];
        /* Calculate 1 character time (μs) = 11 * bit_time_us */
        uint32_t char_time_us = bit_time_us * 11;
        /* Calculate T3.5 = 3.5 * character time (in μs) */
        ...
        /* Convert to 50μs units with ceiling */
