...
void eMBRTUInit(uint32_t ulBaudRate)
{
    ...
    {
        t35_time_us = 1750; // Fixed value: 1750レs
        usTimerT35_50us = 35; // 1750us / 50us = 35
    }
    else
    {
        /* Calculate 1 bit time (レs) = 1,000,000 / baudrate */
        uint32_t bit_time_us = 1000000UL / baud_table[ulBaudRate];
        /* Calculate 1 character time (レs) = 11 * bit_time_us */
        uint32_t char_time_us = bit_time_us * 11;
        /* Calculate T3.5 = 3.5 * character time (in レs) */
        ...
        /* Convert to 50レs units with ceiling */
